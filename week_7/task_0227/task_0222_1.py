...
class DoExcel:
    ...
    def read_date(self,file_name,sheet_name,button):
        ...
        logging.info('---开始读取用例---')
        if button == '1':
            for i in range(2, st.max_row + 1):  # 行数
            ...
            for i in eval(button):  #button不等于1，eval()之后就是一个大列表
                row_list = []
                for j in range(1,st.max_column+1):
                    res=st.cell(i+1,j).value
                    row_list.append(res)
...
if __name__ == '__main__':
    ...
    exa.read_date('test_case.xlsx','Sheet1',button)
