...
import logging
from week_06.homework import read_config

class MyLogger():

    def __init__(self):
        t = read_config.ReadConfig('logconfig.conf')

        self.logger_name = t.read_str('logger', 'logger_name')
        self.logger_level = t.read_str('logger', 'logger_level')

        self.handlerno = t.read_int('handler', 'handlerno')
        self.handler_level = t.read_str('handler', 'handler_level')
        self.formatter = t.read_str('handler','formatter')
        self.filename = t.read_str('handler', 'filename')
