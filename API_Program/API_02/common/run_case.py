...
from API_Program.API_02.common.do_excel import DoExcel
from API_Program.API_02.common.http_request import HttpRequest
from API_Program.API_02.common import project_path
from API_Program.API_02.common.log_test import MyLog
...
class RunCase(unittest.TestCase):
    def setUp(self):
        self.do_exl=DoExcel(project_path.case_path, 'test_case')    # 准备测试数据
        self.my_log=MyLog()
    @data(*test_data)   # 加*解包，相当于遍历test_data传入函数，与加unpack的区别是：加入了unpack后，date里有几个参数，下面的函数就要传入几个变量
    def test_case(self,case):
        ...
        res=HttpRequest()  #实例化
        resp=res.http_request(method,url,param)
        ...
        try:
            ...
            final_result=result
            self.do_exl.write_back(case['CaseId']+1,8,resp.text)   #写实际结果
